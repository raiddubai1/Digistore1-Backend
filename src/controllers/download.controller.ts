...
async function getSignedFileUrl(fileUrl: string, fileName: string): Promise<string> {
  // Check if it's an S3 URL
  const s3Key = getS3KeyFromUrl(fileUrl);
  if (s3Key) {
    // Generate S3 signed URL
    return await getSignedDownloadUrl(s3Key, fileName, 3600); // 1 hour expiry
  }
...
export const downloadFile = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (fileUrl) {
      // Generate signed URL for S3 files
      const fileName = download.product.fileName || 'download';
      const signedUrl = await getSignedFileUrl(fileUrl, fileName);
